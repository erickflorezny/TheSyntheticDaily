    ...
    <div className="min-h-screen bg-[#f9f9f9] text-gray-900 font-serif">
      ...
      <main className="max-w-6xl mx-auto px-4 py-12">
        <div className="border-b-4 border-black pb-4 mb-10">
          <h1 className="text-5xl font-black mb-2">Video</h1>
        ...
        <div className="bg-white border-2 border-black overflow-hidden mb-12">
          <div className="w-full aspect-video bg-gray-900 flex items-center justify-center relative">
            <div className="text-center">
              <div className="w-16 h-16 border-2 border-white/40 flex items-center justify-center mx-auto mb-3">
                <span className="text-white text-2xl ml-1">&#9654;</span>
              </div>
              <span className="text-white/40 font-sans text-xs uppercase tracking-wider">Video Player Placeholder</span>
            </div>
            <span className="absolute bottom-4 right-4 bg-black text-white text-xs font-sans font-bold px-2 py-1">{VIDEOS[0].duration}</span>
          </div>
          <div className="p-6 border-t-2 border-black">
            <span className="text-[10px] font-sans font-bold uppercase tracking-[0.2em] text-gray-500">{VIDEOS[0].category}</span>
            <h2 className="text-2xl font-black mt-1 leading-tight">{VIDEOS[0].title}</h2>
            <p className="text-gray-600 font-sans text-sm mt-2 leading-relaxed">{VIDEOS[0].description}</p>
            <p className="text-gray-400 font-sans text-xs mt-3 uppercase tracking-wider">{VIDEOS[0].views} views</p>
          </div>
        </div>

        {/* Video Grid */}
        <h2 className="text-xl font-black font-sans uppercase border-b-2 border-black pb-2 mb-6 tracking-tight">More Videos</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-0">
          {VIDEOS.slice(1).map(video => (
            <div key={video.title} className="bg-white border border-gray-200 cursor-pointer group">
              <div className="w-full aspect-video bg-gray-800 flex items-center justify-center relative">
                <div className="w-10 h-10 border-2 border-white/30 flex items-center justify-center group-hover:border-white/60 transition">
                  <span className="text-white text-lg ml-0.5">&#9654;</span>
                </div>
                <span className="absolute bottom-2 right-2 bg-black text-white text-[10px] font-sans font-bold px-1.5 py-0.5">{video.duration}</span>
              </div>
              <div className="p-4">
                <span className="text-[10px] font-sans font-bold uppercase tracking-[0.2em] text-gray-400">{video.category}</span>
                <h3 className="text-base font-bold mt-1 leading-snug group-hover:underline">{video.title}</h3>
                <p className="text-gray-400 font-sans text-xs mt-2 uppercase tracking-wider">{video.views} views</p>
              </div>
            </div>
          ))}
        </div>

        <div className="mt-12 border-t-2 border-black pt-4 text-center text-gray-500 font-sans text-xs">
