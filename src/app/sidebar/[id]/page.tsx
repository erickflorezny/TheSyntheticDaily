...
// Sidebar story page — The Onion-style article layout
...
import {
  Zap, Share2, Facebook, Twitter, Link2, Bookmark
} from 'lucide-react';
...
export default async function SidebarStoryPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const story = sidebarStories.find((s) => s.id === parseInt(id));
  if (!story) {
    return <div className="p-8">Sidebar story not found.</div>;
  }

  const otherSidebarStories = sidebarStories.filter(s => s.id !== story.id);
  const mainStories = stories.slice(0, 6);
      ...
      <main className="max-w-6xl mx-auto grid grid-cols-12 gap-8 px-4 py-8">
        ...
        <article className="col-span-12 lg:col-span-7">
          ...
          <div className="mb-4 flex items-center gap-2">
            <a href="#" className="text-purple-700 text-sm font-sans font-bold uppercase tracking-wider hover:underline">
              Sidebar Special
            </a>
            <span className="text-gray-400">/</span>
            <a href="/news" className="text-green-800 text-sm font-sans font-bold uppercase tracking-wider hover:underline">
              {story.tag}
            </a>
          </div>
          ...
          {/* More Sidebar Investigations */}
          <div className="my-12">
            <h3 className="text-xl font-bold mb-4 font-sans border-b-2 border-black pb-2">More Sidebar Investigations</h3>
            <ul className="space-y-4">
              {otherSidebarStories.map(related => (
                <li key={related.id} className="border-b border-gray-200 pb-4">
                  <a href={`/sidebar/${related.id}`} className="group">
                    <div className="flex items-center gap-2">
                      <span className="text-purple-700 text-xs font-sans font-bold uppercase">Sidebar</span>
